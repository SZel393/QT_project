...
if os.path.isfile("systems.sqlite"):
    ...
    con = sqlite3.connect("systems.sqlite")
    cur = con.cursor()
    cur.execute("CREATE TABLE users(id integer PRIMARY KEY, name text, password text, is_teacher iext, date text)")
    cur.execute("INSERT INTO users VALUES(1, 'Lana', '54321', '1', '2020-10-21')")
    cur.execute("INSERT INTO users VALUES(2, 'NoName', '123', '0', '2020-10-21')")
    cur.execute("CREATE TABLE actions(id integer PRIMARY KEY, name text, date text, level text, right text, total text)")
    cur.execute("INSERT INTO actions VALUES(1, 'NoName', '2020-10-21', '0', '0', '0')")
    con.commit()
